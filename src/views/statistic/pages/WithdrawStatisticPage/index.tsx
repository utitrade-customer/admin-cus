...
import { useFetchStatisticWithdraw } from 'hooks';
...
const PAGE = 1;
const SIZE = 10;
const WithdrawStatisticPage = () => {
  const dateFormat = 'MM/DD/YYYY';
  const [timeState, setTimeState] = React.useState({
    start_date: new Date(),
    end_date: new Date(),
  });
  const { data, isFetching, refetch } = useFetchStatisticWithdraw({
          ...
          <Button
            ...
            onClick={() =>
              exportFileExcel({
                ...
                fileName: `Statistic-Withdraw_${timeState.start_date.toDateString()}-${timeState.end_date.toDateString()}`,
              })
            }
          >
            <AiOutlineDownload />
...
export default WithdrawStatisticPage;
