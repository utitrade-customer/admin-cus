...
import { FETCH_WITHDRAW_STATISTIC } from '../constants';
...
export const fetchStatisticWithdraw = (params: FetchStatisticParams) => {
  const { start_date, end_date } = params;
  return API.get(configStatistic)(
    `/withdraw?start_date=${toString(start_date)}&end_date=${toString(
      end_date
    )}`
  );
};

export const useFetchStatisticWithdraw = (params: FetchStatisticParams) => {
  ...
  return useQuery<StatisticsResponse[]>(
    [FETCH_WITHDRAW_STATISTIC, params.start_date, params.end_date],
    () => fetchStatisticWithdraw(params),
    {
